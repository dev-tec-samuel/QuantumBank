...
export class ReceberPixComponent {
  ...
  ngOnInit(): void {
    this.correntistaService.buscarCorrentistaPeloId(this.correntistaService.correntistaLogado).subscribe(
      (dados) => {
        this.correntista = dados; // Atribuindo os dados do correntista
        this.pixKey = this.correntista.chave; // Atribuindo a chave PIX ao campo
      },
      (erro) => {
        console.error('Erro ao buscar correntista:', erro);
      }
    );
  }
  ...
  generateQRCode() {
    ...
    this.qrCodeData = this.generatePixPayload(this.pixKey.replace(/[.-]/g, ''), this.amount, this.description);
  }
